...
class DatagenForm(forms.Form):
    ...
    classroom_num_schedules = forms.IntegerField(label='Number of SCHEDULES for each CL', initial=5)
    classroom_schedule_min_time = forms.DateTimeField(label='Schedule Start Date/Time',
                                                      initial=datetime.now(),
                                                      widget=forms.widgets.DateTimeInput(format='%Y-%m-%d %I:%M %p'),
                                                      input_formats=['%Y-%m-%d %I:%M %p'])
    classroom_schedule_max_time = forms.DateTimeField(label='Schedule End Date/Time',
                                                      initial=datetime.now()+timedelta(weeks=4),
                                                      widget=forms.widgets.DateTimeInput(format='%Y-%m-%d %I:%M %p'),
                                                      input_formats=['%Y-%m-%d %I:%M %p'])
    ...
    ta_num_schedules = forms.IntegerField(label='Number of SCHEDULES for each TA', initial=2)
